...
def test_fit(roi, maptree, response, point_source_model):
    ...
    WCDA.get_saturated_model_likelihood()
    ...
    return jl, WCDA, pts_model, param_df, like_df, data


def test_simulation(test_fit):

    jl, WCDA, pts_model, param_df, like_df, data = test_fit
    
    sim = WCDA.get_simulated_dataset("WCDAsim")
...
def test_plots(test_fit):
    ...
    # Look at the data
    fig = WCDA.display_stacked_image(smoothing_kernel_sigma=0.17)
    # Save to file
    fig.savefig("hal_src_stacked_image.png")

    # Look at the different energy planes (the columns are model, data, residuals)
    fig = WCDA.display_fit(smoothing_kernel_sigma=0.3)
    fig.savefig("hal_src_fit_planes.png")

    fig = WCDA.display_fit(smoothing_kernel_sigma=0.3, display_colorbar=True)
...
def test_goodness(test_fit):
    ...
    print("Prob. of obtaining -log(like) >= observed by chance if null hypothesis is true: %.2f" % gof['WCDA'])
    ...
    plt.axvline(jl.results.get_statistic_frame().loc['WCDA', '-log(likelihood)'],
                color='black',
                linestyle='--')
