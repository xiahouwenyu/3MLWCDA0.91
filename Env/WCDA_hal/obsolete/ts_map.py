...
from WCDA_hal.HAL import HAL
from WCDA_hal.region_of_interest import HealpixConeROI
...
class ParallelTSmap(object):
    ...
    def __init__(self, maptree, response, ra_c, dec_c,  xsize, ysize, pix_scale, projection = 'AIT', roi_radius=3.0, s=0, e=6, piv=3, index=-2.6, indexf=False):
        ...
        roi = HealpixConeROI(self._roi_radius, self._roi_radius+1, ra=ra_c, dec=dec_c)
        
        self._llh = HAL("WCDA", self._mtfile, self._rsfile, roi)
        self._llh.set_active_measurements(s,e)
    ...
    def get_model(self, interval_id):
        ...
        spectrum.piv = self.piv * u.TeV
        spectrum.piv.fix = True
                
        # Start from a flux 1/10 of the Crab
        spectrum.K = crab_diff_flux_at_1_TeV
        spectrum.K.bounds = (1e-30, 1e-10)
        spectrum.index = self.index
        spectrum.index.fix = self.indexf
    ...
    def worker(self, interval_id):
        ...
        return like['-log(likelihood)']['WCDA']
