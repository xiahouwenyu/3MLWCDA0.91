...
import numpy as np
import pytest
from WCDA_hal import HAL, HealpixConeROI
from threeML import *
...
def fit_point_source(roi, maptree, response, point_source_model, liff=False):
    ...
    if not liff:
        ...
        WCDA = WCDALike("WCDA", maptree, response, fullsky=True)
    ...
    if not liff:
        ...
        WCDA.display_fit(display_colorbar=True).savefig("display_fit.png")
...
def roi():
    ...
    # zebra-source-injector -i /home/giacomov/science/WCDA/data/zebra/combined_bin{0..9}.fits.gz -o bin{0..9}.fits.gz
    # -b {0..9}
