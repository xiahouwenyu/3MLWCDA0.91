import pytest
from WCDA_hal import HealpixMapROI, HAL
...
def test_healpixRoi(geminga_maptree,geminga_response):
  ...
  WCDA = HAL("WCDA",geminga_maptree,geminga_response,map_roi)
  ...
  WCDA.set_model(model)

  # Write the model map
  model_map_tree=WCDA.write_model_map("test.hd5",test_return_map=True)

  # Read the model back 
  WCDA_model = map_tree_factory('test.hd5',map_roi)
