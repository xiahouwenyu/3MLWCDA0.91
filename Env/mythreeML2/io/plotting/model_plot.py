...
class SpectralContourPlot(object):
    ...
    def add_model(
        ...
        if "scale" in self._plot_kwargs.keys():
            scale=self._plot_kwargs["scale"]
            self._plot_kwargs.pop("scale")
        else:
            scale=1
        
        if "index" in self._plot_kwargs.keys():
            index=self._plot_kwargs["index"]
            self._plot_kwargs.pop("index")
        else:   
            index=-2

        if "color" in self._plot_kwargs.keys():
            if "label" in self._plot_kwargs.keys():
                self._ax.plot(
                    energy_range, energy_range**(-index-2)*best_fit*scale, **self._plot_kwargs
                )
            else:
                self._ax.plot(
                    energy_range, energy_range**(-index-2)*best_fit*scale, label=label, **self._plot_kwargs
                )

            if (upper_error is not None) and (lower_error is not None):
                self._ax.fill_between(
                    energy_range,
                    energy_range**(-index-2)*lower_error*scale,
                    energy_range**(-index-2)*upper_error*scale,
                    facecolor=self._plot_kwargs["color"],
                    **self._contour_kwargs
                )
        else:
            if "label" in self._plot_kwargs.keys():
                self._ax.plot(
                    energy_range, energy_range**(-index-2)*best_fit*scale, color=color, **self._plot_kwargs
                )
            else:
                self._ax.plot(
                    energy_range, energy_range**(-index-2)*best_fit*scale, color=color, label=label, **self._plot_kwargs
                )
