...
class JointLikelihood(object):
    ...
    def get_contours(
        ...
        **options,
        ...
        try:
            ...
            min1, max1 = param_1_minimum, param_1_maximum
    ...
    def minus_log_like_profile(self, *trial_values):
        ...
        if self.verbose:
            ...
            # Calculate iteration count and percentage
            iteration_count = self._ncalls
            total_iterations = 400000
            percentage_complete = (iteration_count / total_iterations) * 100

            # Estimate time remaining
            if hasattr(self, '_intervals') and len(self._intervals) > 0:
                estimated_days_remaining = ((total_iterations - iteration_count) * average_interval) / (24 * 3600)
                log.info(f"Iteration: {iteration_count}, Completion: {percentage_complete:.2f}%, Estimated time remaining: {estimated_days_remaining:.2f} days")
